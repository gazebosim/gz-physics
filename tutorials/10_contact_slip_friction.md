...
- Compliant friction model example
    ...
    - A more compliant friction model is generated by applying a constraint
      ...
      friction force to represent a saturated linear function.
    - For small values of the slip velocity `v_t`, model the friction force as
      a linear function of slip velocity with linear coefficient
      `slip_stiffness` as `T = - slip_stiffness * v_t` or alternatively the
      inverse coefficient `slip_compliance` as `T = - v_t / slip_compliance`.
    - The magnitude of `T` is still limited to `µN`, resulting in the
      saturated linear function illustrated in the plot below.
      Let `v_s` represent the slip velocity at which the friction force
      saturates as `v_s = µN / slip_stiffness` or equivalently
      `v_s = µN * slip_compliance`.
    - For the step function in the previous plot, the slope
      at the origin is infinite, whereas a finite slope allows friction forces
      less than the maximum value to be applied for small slip values.
    - For the relaxed model, the slope at the origin is the "slip stiffness",
      while the inverse slope at the origin is the "slip compliance."
