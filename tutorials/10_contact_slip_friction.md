...
- Compliant friction model example
    ...
    - Let `sat(x)` represent a unit saturation function: `sat(x) = x ∀ |x|≤1`,
      `sat(x) = sgn(x) ∀ |x|>1`.
    - The friction model is relaxed to use the linear saturation function as
      `T = -µN sat(v_t / v_s)`, with `v_s` representing the slip velocity at
      which the friction force reaches saturation. The relaxed model is shown
      in the plot belo. For the step function in the previous plot, the slope
    ...
    - For the relaxed model, the slope at the origin is `-µN / v_s` and
      represents "slip stiffness." The inverse slope at the origin is
      `-v_s / (µN)` and represents "slip compliance."
