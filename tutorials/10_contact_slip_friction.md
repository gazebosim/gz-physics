...
Contact mechanics are complex, and high-fidelity models using finite elements
are not feasible for use in real-time simulation.

Gazebo and most of its currently supported physics engines make some
approximations to model contact to reduce the computation required for
simulation, which are summarized in the following table:

Gazebo's contact model | Reality
---------------------- | ------------------------
**Nondeformable**: objects are completely rigid geometric shapes                                                                        | **Deformable**: objects can change shape when force is applied
In contact, object geometries have **volumetric overlap** / interpenetration                                                            | In contact, object geometries **flatten slightly** to prevent volumetric overlap
Contact is represented by a **set of contact points** and normal directions inside the overlapping volume or on the surface of objects  | Contact is represented by a **contact patch** where flattened surfaces are touching
Equal and opposite **forces** act on both bodies at each contact point                                                                  | **Contact pressure** and **shear stress** act over the surface of each contact patch

Gazebo's approximations of contact can give similar results to more complex
contact mechanics models if the wrenches applied at the center of mass of each
object by the sum of forces acting at contact points are equivalent to the
wrenches generated by the contact pressure and shear stress at contact patches.

In particular, the similarity may be improved when contact points are located
at the center of pressure of a contact patch.
