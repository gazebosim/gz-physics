...
TEST(Collision, BoxShape)
{
  Collision collision;
  BoxShape boxShape;
  boxShape.SetSize(gz::math::Vector3d(0.5, 0.5, 0.5));
  collision.SetShape(boxShape);
  auto result = collision.GetShape();
  ASSERT_NE(nullptr, result);
  EXPECT_EQ(gz::math::Vector3d(0.25, 0.25, 0.25),
...
TEST(Collision, CapsuleShape)
{
  ...
  auto result = collision.GetShape();
  ASSERT_NE(nullptr, result);
  EXPECT_EQ(gz::math::Vector3d(2.0, 2.0, 3.5),
      result->GetBoundingBox().Max());
}
...
TEST(Collision, CylinderShape)
{
  ...
  auto result = collision.GetShape();
  ASSERT_NE(nullptr, result);
  EXPECT_EQ(gz::math::Vector3d(2.0, 2.0, 1.5),
      result->GetBoundingBox().Max());
}
...
TEST(Collision, EllipsoidShape)
{
  ...
  auto result = collision.GetShape();
  ASSERT_NE(nullptr, result);
  EXPECT_EQ(gz::math::Vector3d(1.0, 2.0, 1.3),
      result->GetBoundingBox().Max());}
...
TEST(Collision, SphereShape)
{
  ...
  auto result = collision.GetShape();
  ASSERT_NE(nullptr, result);
  EXPECT_EQ(gz::math::Vector3d(2.0, 2.0, 2.0),
      result->GetBoundingBox().Max());
}
