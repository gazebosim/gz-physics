...
Identity SDFFeatures::ConstructSdfModelImpl(const Identity& _parentID,
  ...
  ignerr << "Model " << name << ": " << sdfPose << std::endl;

  // Get model pose wrt world
  math::Pose3d worldPose;
  if (_parentID.id == this->worldId) {
    // Top level model
    worldPose = sdfPose;
  } else {
    // Nested model
    auto parentModel = std::get<Model*>(this->entities.at(_parentID));
    worldPose = parentModel->worldPose * sdfPose;
  }

  // Create the model
  auto model =
      std::make_unique<Model>(name, sdfPose, worldPose, isStatic, this->world->btWorld.get());
...
Identity SDFFeatures::ConstructSdfLink(const Identity& _modelID,
  ...
  auto sdfPose = ResolveSdfPose(_sdfLink.SemanticPose());
  auto inertial = _sdfLink.Inertial();

  ignerr << "Link Model " << model->name << ": " << model->sdfPose << std::endl;
  ...
  auto poseIsometry =
      ignition::math::eigen3::convert(model->worldPose * sdfPose * inertialPose);
  ...
  auto link = std::make_unique<Link>(name, sdfPose, inertialPose, model->world,
...
Identity SDFFeatures::ConstructSdfJoint(const Identity& _modelID,
  ...
  if (parentLink != nullptr) {
    pivotParent =
        (childLink->sdfPose * ResolveSdfPose(_sdfJoint.SemanticPose())).Pos();
