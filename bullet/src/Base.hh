...
/////////////////////////////////////////////////
struct Model {
  Model(std::string _name, const math::Pose3d &_sdfPose, bool _static,
...
/////////////////////////////////////////////////
struct Link {
  Link(std::string _name, const math::Pose3d &_sdfPose,
       const math::Pose3d &_sdfInertialPose, btDiscreteDynamicsWorld *_btWorld,
       const btTransform &_baseTransform, double _mass,
...
/////////////////////////////////////////////////
struct Collision {
  Collision(std::string _name, btRigidBody *_body, bool _isMesh,
            std::unique_ptr<btCollisionShape> &&_shape,
            std::unique_ptr<btTriangleMesh> &&_mesh, const btTransform &_pose,
            double _mu = .0, double _mu2 = .0);
...
/////////////////////////////////////////////////
struct Joint {
  Joint(std::string _name, btDiscreteDynamicsWorld *_world, btRigidBody *_childBody,
        btRigidBody *_parentBody, const btVector3 &_pivotChild,
        const btVector3 &_pivotParent, const btVector3 &_axisChild,
        const btVector3 &_axisParent, ::sdf::JointType _type);
...
class Base : public Implements3d<FeatureList<Feature>> {
  ...
  public: std::unique_ptr<World> world;

  /// \ brief The World ID
  public: std::size_t worldId;

  /// \brief Hashmap for fast access to an Element given the ID
  public: std::unordered_map<std::size_t, EntityPtr> entities;

  /// \brief Hashmap for fast access to a Container given its child ID
  public: std::unordered_map<std::size_t, EntityPtr> containers;

  /// \brief Counter for generating IDs
  private: std::size_t entityCount = 0;

  /// \brief Gets the next available ID
  public: inline std::size_t GetNextEntity() { return entityCount++; }
  ...
  public: inline Identity AddModel(std::unique_ptr<Model> &&_model) {
    auto id = this->GetNextEntity();
  ...
  public: inline Identity AddNestedModel(std::size_t _modelId,
  ...
  public: inline Identity AddLink(std::size_t _modelId, std::unique_ptr<Link> &&_link) {
    auto id = this->GetNextEntity();
    auto model = std::get<Model *>(this->entities.at(_modelId));
  ...
  public: inline Identity AddJoint(std::size_t _modelId,
