...
/////////////////////////////////////////////////
Model::Model(std::string _name, const math::Pose3d &_sdfPose,
             bool _static, btDiscreteDynamicsWorld *_btWorld)
    : name(std::move(_name)), sdfPose(_sdfPose), fixed(_static), world(_btWorld) {}

/////////////////////////////////////////////////
Link::Link(std::string _name, const math::Pose3d &_sdfPose,
           const math::Pose3d &_sdfInertialPose,
           btDiscreteDynamicsWorld *_btWorld, const btTransform &_baseTransform,
           double _mass, const btVector3 &_linkInertiaDiag)
    : name(std::move(_name)),
      sdfPose(_sdfPose),
...
/////////////////////////////////////////////////
Link::~Link() { world->removeRigidBody(body.get()); }

/////////////////////////////////////////////////
Collision::Collision(std::string _name, btRigidBody *_body, bool _isMesh,
                     std::unique_ptr<btCollisionShape> &&_shape,
                     std::unique_ptr<btTriangleMesh> &&_mesh,
                     const btTransform &_pose, double _mu, double _mu2)
    : name(std::move(_name)),
      body(_body),
      pose(_pose),
      isMesh(_isMesh),
  ...
  dynamic_cast<btCompoundShape *>(body->getCollisionShape())
      ->addChildShape(pose, collisionShapePtr);
}
...
/////////////////////////////////////////////////
Joint::Joint(std::string _name, btDiscreteDynamicsWorld *_world, btRigidBody *_childBody,
  ...
  if (type == ::sdf::JointType::FIXED) {
    auto *hingeConstraint =
        static_cast<btHingeAccumulatedAngleConstraint *>(constraint.get());
    auto offset = hingeConstraint->getHingeAngle();
    hingeConstraint->setLimit(offset, offset);
  }
...
/////////////////////////////////////////////////
Joint::~Joint() { world->removeConstraint(constraint.get()); }
