...
namespace gz
{
  namespace physics
  {
    namespace bullet_featherstone
    {
      ...
      public:
        void SetJointPosForDof(
            int _jointIndex,
            std::size_t _dof,
            btScalar _value);

      private:
        using btMultiBody::setJointPos;

      private:
        using btMultiBody::setJointPosMultiDof;

      private:
        using btMultiBody::getJointPosMultiDof;
      ...
      public:
        btScalar GetJointPosForDof(int _jointIndex, std::size_t _dof) const;
      ...
      public:
        void SetBaseWorldTransform(const btTransform &_pose);

      private:
        using btMultiBody::setBaseWorldTransform;

        /// \brief Update collision transforms if `needsCollisionTransformsUpdate` is
        /// set and reset the flag.
      public:
        void UpdateCollisionTransformsIfNeeded();
      ...
      public:
        void AddJointDampingStiffnessTorque(int _jointIndex, double _damping,
                                            double _springStiffness, double _springReference);

      private:
        bool needsCollisionTransformsUpdate = false;
      };
      ...
      public:
        bool checkCollideWithOverride(const btCollisionObject *_co) const
            override
        {
          ...
          const LinkInfo &linkInfo)
      {
        ...
        {
          return convert(GetWorldTransformOfLinkInertiaFrame(body, *indexOpt)) * linkInfo.inertiaToLinkFrame;
        }
      ...
      public:
        inline Identity AddWorld(WorldInfo _worldInfo)
        {
      ...
      public:
        inline Identity AddNestedModel(
            std::string _name,
            Identity _parentID,
            Identity _worldID,
            Eigen::Isometry3d _rootLinkToModelTf,
      ...
      public:
        inline Identity AddLink(LinkInfo _linkInfo)
        {
          const auto id = this->GetNextEntity();
      ...
      public:
        inline Identity AddCollision(CollisionInfo _collisionInfo)
        {
          const auto id = this->GetNextEntity();
      ...
      public:
        inline Identity AddJoint(JointInfo _jointInfo)
        {
          const auto id = this->GetNextEntity();
      ...
      public:
        inline Identity addConstraint(JointInfo _jointInfo)
        {
          const auto id = this->GetNextEntity();
          auto joint = std::make_shared<JointInfo>(std::move(_jointInfo));
          this->joints[id] = joint;
      ...
      public:
        bool RemoveModelImpl(const Identity &_parentID,
                             const Identity &_modelID)
        {
          ...
          // map entries, and the btMultiBody itself.
          if (!isNested)
          {
      ...
      public:
        using WorldInfoPtr = std::shared_ptr<WorldInfo>;
      ...
      public:
        using JointInfoPtr = std::shared_ptr<JointInfo>;

      public:
        std::unordered_map<std::size_t, WorldInfoPtr> worlds;

      public:
        std::unordered_map<std::size_t, ModelInfoPtr> models;

      public:
        std::unordered_map<std::size_t, LinkInfoPtr> links;

      public:
        std::unordered_map<std::size_t, CollisionInfoPtr> collisions;

      public:
        std::unordered_map<std::size_t, JointInfoPtr> joints;

      public:
        std::vector<std::unique_ptr<btTriangleMesh>> triangleMeshes;

      public:
        std::vector<std::unique_ptr<btGImpactMeshShape>> meshesGImpact;

      public:
        std::vector<std::unique_ptr<btConvexHullShape>> meshesConvex;
      };
