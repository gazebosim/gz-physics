...
{
  const auto *shapeInfo = this->ReferenceInterface<CollisionInfo>(_shapeID);
  const auto *trimeshShape = dynamic_cast<const btBvhTriangleMeshShape*>(shapeInfo->collider.get());
  auto v = trimeshShape->getLocalAabbMax() - trimeshShape->getLocalAabbMin();
  return math::eigen3::convert(math::Vector3d(v[0], v[1], v[2]));
}
      ...
      const Identity &_shapeID) const 
{
  const auto *shapeInfo = this->ReferenceInterface<CollisionInfo>(_shapeID);
  const auto *trimeshShape = dynamic_cast<const btBvhTriangleMeshShape*>(shapeInfo->collider.get());
  auto v = trimeshShape->getLocalScaling();
  return math::eigen3::convert(math::Vector3d(v[0], v[1], v[2]));
}
*/
...
{
  printf("ShapeFeatures::AttachMeshShape\n");
  ...
  const auto numVertices = _mesh.VertexCount();
  const auto numIndices = _mesh.IndexCount();

  auto triMesh = std::make_shared<btTriangleMesh>();
  ...
  std::unique_ptr<btBvhTriangleMeshShape> shape =
    std::make_unique<btBvhTriangleMeshShape>(triMesh.get(), true, true);
